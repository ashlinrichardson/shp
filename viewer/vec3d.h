...
class vec3d{  public:
        ...
        vec3d  operator+(float s){vec3d ret( x+s,y+s,z+s); return ret;}
        vec3d  operator-(float s){vec3d ret( x-s,y-s,z-s); return ret;}
        vec3d  operator*(float s){vec3d ret( x*s, y*s, z*s);return ret;}
        vec3d  operator/(float s){vec3d ret( x/s, y/s ,z/s);return ret;}
        float length(){return sqrt( x*x +y*y +z*z);}

        inline void vertex(){glVertex3f( x, y, z);}
        inline void vertex2f(){ glVertex2f(x,y);}
        inline void color(){ glColor3f(x,y,z); }
